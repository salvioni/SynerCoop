...
router.get('/tenants', async (_req, res, next) => {
  try {
    ...
    const stats = await db.prepare(`
      SELECT tenant_id, COUNT(*) AS managers
    ...
    const statMap     = Object.fromEntries(stats.map(s => [s.tenant_id, s]));
    const clientMap   = Object.fromEntries(clientCounts.map(x => [x.tenant_id, x.c]));
    const analysisMap = Object.fromEntries(analysisCounts.map(x => [x.tenant_id, x.c]));
...
// PATCH /admin/tenants/:id/toggle — ativa ou desativa um tenant
...
// PATCH /admin/tenants/:id/plan — altera plano de um tenant (manual/Enterprise)
router.patch('/tenants/:id/plan', async (req, res, next) => {
  try {
    const { id } = req.params;
    const { plan } = req.body;
    const VALID_PLANS = ['trial', 'basic', 'pro', 'enterprise', null];
    if (!VALID_PLANS.includes(plan)) throw badRequest('Plano inválido.');
    const tenant = await db.prepare('SELECT id FROM tenants WHERE id = ?').get(id);
    if (!tenant) throw badRequest('Tenant não encontrado.');
    await db.prepare(
      `UPDATE tenants SET plan = ?, onboarded_at = COALESCE(onboarded_at, CURRENT_TIMESTAMP) WHERE id = ?`
    ).run(plan, id);
    res.json({ ok: true, plan });
  } catch (e) { next(e); }
});

// POST /admin/tenants/:id/invite — convida gerente para um tenant
router.post('/tenants/:id/invite', async (req, res, next) => {
  try {
    ...
    // Corrigido: devLink só exposto fora de produção (igual às demais rotas de invite)
